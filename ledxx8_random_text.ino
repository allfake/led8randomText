...
void loop() {
  ...
  char* currentMessage[8];
  
  currentMessage[0] = "60 % OF INFECTIOUS DISEASES IN HUMANS COME FROM ANIMALS    ";
  currentMessage[1] = "ALL RECENT PANDEMIC FLU STRAINS CONTAINED ANIMALS GENES   ";
  currentMessage[2] = "SEASONAL FLU KILLS BETWEEN 250-500 K PEOPLE EACH YEAR   ";
  currentMessage[3] = "DENGUE IS THE LEADING CAUSE OF ILLNESS & DEATH IN THE TROPICS AND SUBTROPICS   ";
  currentMessage[4] = "A UNIVERSAL FLU VACCINE MAY BE DEVELOPED BY 2018   ";
  currentMessage[5] = "SMALLPOX IS THE ONLY HUMAN DISEASE THAT HAS BEEN FULLY ERADICATED   ";
  currentMessage[6] = "223 CASES OF POLIO IN 2012 WAS THE LOWEST TOTAL IN HISTORY   ";
  currentMessage[7] = "AIR TRAVEL CAN SPREAD INFECTIOUS DISEASES ANYWHERE IN LESS THAN 36   ";

  for(int i = 0; i < strlen(currentMessage[randNumber]); i++) {
    charMessage[i] = currentMessage[randNumber][i];
  }
  charMessageCurrent = strlen(currentMessage[randNumber]);
